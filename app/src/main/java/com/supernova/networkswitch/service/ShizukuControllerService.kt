...
class ShizukuControllerService() : IShizukuController.Stub() {
    ...
    init {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
            try {
                ...
                Log.e("NetworkSwitch", "Shizuku: HiddenApiBypass failed", e)
            }
        }
    }
    ...
    companion object {
        ...
        private fun getITelephony(): Any? {
            return try {
                ...
                Log.e(TAG, "Shizuku: Failed to get ITelephony", e)
                null
            }
        }
        ...
        private fun mapBitmaskToNetworkMode(bitmask: Long): Int {
            ...
            // Handle cases where bitmask might have additional bits (like LTE_CA vs LTE)
    ...
    override fun getCurrentNetworkMode(subId: Int): Int {
        try {
            ...
            for (m in methods) {
                if (m.name == "getAllowedNetworkTypesForReason") {
                    try {
                        ...
                        Log.i(TAG, "Shizuku: getCurrentNetworkMode(subId=$subId) -> $mode (bitmask=$bitmask)")
                        return mode
                    } catch (_: Exception) {
            ...
            Log.e(TAG, "Shizuku: Error in getCurrentNetworkMode", e)
        }
        return -1
    }
    ...
    override fun setNetworkMode(subId: Int, networkMode: Int) {
        try {
            ...
            for (m in methods) {
                if (m.name == "setAllowedNetworkTypesForReason") {
                    try {
                        ...
                        if (success) {
                            Log.i(TAG, "Shizuku: setAllowedNetworkTypesForReason(subId=$subId, mode=$networkMode) success")
                            break
                        }
                    } catch (_: Exception) {
                        // Try next
                ...
                for (m in methods) {
                    if (m.name == "setAllowedNetworkTypes") {
                        try {
                            ...
                            if (success) {
                                Log.i(TAG, "Shizuku: setAllowedNetworkTypes(subId=$subId, bitmask=$bitmask) success")
                                break
                            }
                        } catch (_: Exception) {
                            // Try next
                ...
                for (m in methods) {
                    if (m.name == "setPreferredNetworkType" && m.parameterCount == 2) {
                        try {
                            ...
                            success = true
                            Log.i(TAG, "Shizuku: setPreferredNetworkType(subId=$subId, mode=$networkMode) success")
                            break
                        } catch (_: Exception) {
                            // Try next
                        }
            ...
            if (!success) {
                Log.w(TAG, "Shizuku: Failed to set network mode $networkMode for subId $subId")
            }

        } catch (e: Throwable) {
            Log.e(TAG, "Shizuku: FATAL error in setNetworkMode", e)
        }
    }
